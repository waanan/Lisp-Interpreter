...
(define-datatype continuation continuation?
  ...
  (rator-cont
   ...
   (cont continuation?))
  (rand-cont
   (val1 (lambda (x) (or (expval? x) (continuation? x))))
   (cont continuation?)))
(define apply-cont
  (lambda (cont val)
      ...
      (callCC-cont (env cont)
                   (let ((proc1 (expval->proc val)))
                     (apply-procedure/k proc1
                                        (proc-val (procedure 'v (call-exp (cont-exp cont) (var-exp 'v)) env)) cont)))
    ...
    (cases expression exp
      ...
      (const2-exp (bool) (apply-cont cont
                                     (if (eq? bool 't)
                                         (bool-val #t)
                                         (bool-val #f))))
      (cont-exp (cont1) (apply-cont cont cont1))
      ...
      (callCC-exp (exp1)
                  (value-of/k exp1 env (callCC-cont env cont)))
...
;                         then (y (x z))
;                         else 42)")
;(run "let callWithCurrentContinuation
;              = proc (p)
;                     letcc cont 
;                           (p proc (v) (cont v))
;         in let q = #t
...
;                          else 2
;                 in let z = 1
;                     in -(z,(callWithCurrentContinuation dec))")
