#lang eopl
(define count-help
  (lambda (max-count cur-count)
    (lambda (x)
      (cond ((equal? x 'reset) (set! max-count 0)
                          (set! cur-count 0))
            ((equal? x 'add) (set! cur-count (+ cur-count 1))
                        (cond ((> cur-count max-count) (set! max-count cur-count))))
            ((equal? x 'dec) (set! cur-count (- cur-count 1)))
            ((equal? x 'cur) cur-count)
            ((equal? x 'max) max-count)))))
(define count-init!
  (count-help 0 0))
(define count-add!
  (lambda () (count-init! 'add)))
(define count-dec!
  (lambda () (count-init! 'dec)))
(define count-reset!
  (lambda () (count-init! 'reset)))
(define count-max
  (lambda () (count-init! 'max)))
(define count-cur
  (lambda () (count-init! 'cur)))
  ...
  (lambda ()
    ...
    (lambda (val)
      (begin 
        ...
        (eopl:printf "Max of contination is ~s.~%" (count-max))
        (eopl:printf "Cur of contination is ~s.~%" (count-cur))
        val))))
(define null?-cont 
  (lambda (saved-cont)
    (count-add!)
...
(define list-cont 
  (lambda (expvals exps env saved-cont)
    (count-add!)
    (lambda (val)
      (if (null? exps)
...
(define let-exp-cont 
  (lambda (var vars exps body const-env saved-env saved-cont)
    (count-add!)
    (lambda (val)
      (if (null? vars)
          (value-of/k body
...
(define rands-cont 
  (lambda (val1 rands rest-rands env cont)
    (count-add!)
    (lambda (val) 
      (if (null? rest-rands)
