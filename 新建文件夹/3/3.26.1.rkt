#lang eopl
;;exercise 3.26
;;modify the procedure.not pass the env.
(define empty-env
                ...
                (let ((val1 (value-of exp1 env))
                  ...
                  (let ((num1 (expval->num val1))
                    ...
                    (bool-val
                     (< num1 num2)))))
      (proc-exp (vars body)
                (proc-val (procedure vars (env->body body env vars))))
...
(define apply-procedure
  (lambda (proc1 rands now-env)
    (cases proc proc1
      (procedure (vars body)
                     (apply-help
                      vars rands  body now-env)))))
(define apply-help
  (lambda (vars rands body now-env)
    (cond ((and (null? vars) (not (null? rands)))
          ...
          ((and (null? rands) (null? vars))
           (value-of body now-env))
          (else
           (let ((var (car vars))
                 (arg (value-of (car rands) now-env))
                 (rest-vars (cdr vars))
                 (rest-rands (cdr rands)))
             (apply-help rest-vars rest-rands body (extend-env var arg now-env))))))) 
(define env->body
  (lambda (exp env lst)
      (cases expression exp
        (const1-exp (num) (const1-exp num))
        (const2-exp (bool) (const2-exp bool))
        (var-exp (var) (cond ((member var lst) (var-exp var))
                             (else (cases expval (apply-env env var)
                                     (num-val (num)
                                              (const1-exp num))
                                     (bool-val (bool) 
                                               (const2-exp bool))
                                     (proc-val (procd)
                                               (cases proc procd
                                                 (procedure 
                                                  ...
                                                  (proc-exp vars body))))))))
        (construct-exp (exp1 exp2)
                       (construct-exp (env->body exp1 env lst) (env->body exp2 env lst)))
        (emptylist ()
                   (emptylist))
        (car-exp (exp1)
                 (car-exp (env->body exp1 env lst)))
        (cdr-exp (exp1)
                 (cdr-exp (env->body exp1 env lst)))
        (list-exp (exps)
                  (list-exp (map (lambda (exp) (env->body exp env lst)) exps)))
        (null?-exp (exp1)
                   (null?-exp (env->body exp1 env lst))) 
        (diff-exp (exp1 exp2)
                  (diff-exp (env->body exp1 env lst) (env->body exp2 env lst)))
        (zero?-exp (exp1)
                   (zero?-exp (env->body exp1 env lst)))
        (if-exp (exp1 exp2 exp3)
                (if-exp (env->body exp1 env lst) (env->body exp2 env lst) (env->body exp3 env lst)))
        (cond-exp (exps1 exps2)
                  (cond-exp (map (lambda (exp) (env->body exp env lst)) exps1) (map (lambda (exp) (env->body exp env lst)) exps2)))
        (let-exp (lst1 lst2 body)
                 (let-exp (map (lambda (exp) (env->body exp env lst)) lst1) (map (lambda (exp) (env->body exp env lst)) lst2) (env->body body env lst)))
        (let*-exp (lst1 lst2 body)
                  (let*-exp (map (lambda (exp) (env->body exp env lst)) lst1) (map (lambda (exp) (env->body exp env lst)) lst2) (env->body body env lst)))
        (unpack-exp (lst1 exp1 exp2)
                    (unpack-exp (map (lambda (exp) (env->body exp env lst)) lst1) (env->body exp1 env lst) (env->body exp2 env lst)))
        (minus-exp (exp1)
                   (minus-exp (env->body exp1 env lst)))
        (add-exp (exp1 exp2)
                 (add-exp (env->body exp1 env lst) (env->body exp2 env lst)))
        (multi-exp (exp1 exp2)
                   (multi-exp (env->body exp1 env lst) (env->body exp2 env lst)))
        (quoti-exp (exp1 exp2)
                   (quoti-exp (env->body exp1 env lst) (env->body exp2 env lst)))
        (equal?-exp (exp1 exp2)
                    (equal?-exp (env->body exp1 env lst) (env->body exp2 env lst)))
        (greater?-exp (exp1 exp2)
                      (greater?-exp (env->body exp1 env lst) (env->body exp2 env lst)))
        (less?-exp (exp1 exp2)
                   (less?-exp (env->body exp1 env lst) (env->body exp2 env lst)))
        (proc-exp (vars1 body)
                  (proc-exp vars1 (env->body body env (append vars1 lst))))
        (call-exp (rator rands)
                  (call-exp (env->body rator env lst) (map (lambda (exp) (env->body exp env lst)) rands))))))
;;exercise3.23 fact
(define fact
