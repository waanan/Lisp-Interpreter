  ...
  (lambda (env1 env2)
    (if (empty?-env env1)
        ...
        (let ((saved-var (cadr env1))
          ...
          (extend-env saved-var saved-val (append-env saved-env env2))))))
(define flatmap-env
  (lambda (f envs)
    (flatmap-help (map f envs) (empty-env))))
(define flatmap-help
  (lambda (envs receive)
    (if (null? envs)
        receive
        (flatmap-help (cdr envs) (append-env (car envs) receive))))) 
                ...
                (let ((val1 (value-of exp1 env))
                  ...
                  (let ((num1 (expval->num val1))
                    ...
                    (bool-val
                     (< num1 num2)))))
      (proc-exp (vars body)
                (proc-val (procedure vars body (body->free-env body env vars))))
...
(define body->free-env
  (lambda (exp env lst)
      (cases expression exp
        (const1-exp (num) (empty-env))
        (const2-exp (bool) (empty-env))
        (var-exp (var) (cond ((member var lst) (empty-env))
                             (else (cases expval (apply-env env var)
                                     ...
                                     (proc-val (proc)
                                               (extend-env var (apply-env env var) (empty-env)))))))
        (construct-exp (exp1 exp2)
                       (append-env (body->free-env exp1 env lst) (body->free-env exp2 env lst)))
        (emptylist ()
                   (empty-env))
        (car-exp (exp1)
                 (body->free-env exp1 env lst))
        (cdr-exp (exp1)
                 (body->free-env exp1 env lst))
        (list-exp (exps)
                  (flatmap-env (lambda (exp) (body->free-env exp env lst)) exps))
        (null?-exp (exp1)
                   (body->free-env exp1 env lst)) 
        (diff-exp (exp1 exp2)
                  (append-env (body->free-env exp1 env lst) 
                              (body->free-env exp2 env lst)))
        (zero?-exp (exp1)
                    (body->free-env exp1 env lst)) 
        (if-exp (exp1 exp2 exp3)
                (append-env (append-env (body->free-env exp1 env lst) 
                                        (body->free-env exp2 env lst)) 
                            (body->free-env exp3 env lst)))
        (cond-exp (exps1 exps2)
                 (append-env (flatmap-env (lambda (exp) (body->free-env exp env lst)) exps1)
                             (flatmap-env (lambda (exp) (body->free-env exp env lst)) exps2)))
        (let-exp (lst1 lst2 body)
                 (append-env (append-env (flatmap-env (lambda (exp) (body->free-env exp env lst)) lst1) 
                                         (flatmap-env (lambda (exp) (body->free-env exp env lst)) lst2)) 
                             (body->free-env body env lst)))
        (let*-exp (lst1 lst2 body)
                  (append-env (append-env (flatmap-env (lambda (exp) (body->free-env exp env lst)) lst1) 
                                          (flatmap-env (lambda (exp) (body->free-env exp env lst)) lst2)) 
                              (body->free-env body env lst)))
        (unpack-exp (lst1 exp1 exp2)
                    (append-env (flatmap-env (lambda (exp) (body->free-env exp env lst)) lst1) 
                                (append-env (body->free-env exp1 env lst) 
                                            (body->free-env exp2 env lst))))
        (minus-exp (exp1)
                   (body->free-env exp1 env lst))
        ...
        (less?-exp (exp1 exp2)
                     (append-env (body->free-env exp1 env lst) 
                                 (body->free-env exp2 env lst)))
        (proc-exp (vars1 body)
                  (body->free-env body env (append vars1 lst)))
        (call-exp (rator rands)
                  (append-env (body->free-env rator env lst) (flatmap-env (lambda (exp) (body->free-env exp env lst)) rands))))))
;;exercise3.23 fact
(define fact
