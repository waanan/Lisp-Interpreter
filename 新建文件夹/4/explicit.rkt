...
(define translation-of
  (lambda (exp senv)
    (cases expression exp
      ...
      (newref-exp (exp1)
                  (newref-exp (translation-of exp1 senv)))
      (deref-exp (exp1)
                 (deref-exp (translation-of exp1 senv)))
      (setref-exp (exp1 exp2)
                  (setref-exp (translation-of exp1 senv)
                              (translation-of exp2 senv)))
      (beg-exp (exps)
      ...
      (deref-exp (exp1)
                 (let ((v1 (value-of exp1 nameless-env)))
                   (let ((ref1 (expval->ref v1)))
                     (deref ref1))))
      (setref-exp (exp1 exp2)
                  (let ((ref (expval->ref (value-of exp1 nameless-env))))
                    (let ((val (value-of exp2 nameless-env)))
